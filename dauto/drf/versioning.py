...
class CustomNamespaceVersioning(NamespaceVersioning):
    ...
    Methods: get_versioned_viewname(viewname: str, request) -> str: This method takes a view name and a request
    object as parameters and returns the versioned view name. If the view name contains the separator, it splits the
    view name and version using the separator attribute and returns the versioned view name in the format "{
    version}:{view}". If the view name does not contain the separator, it delegates the call to the parent class's
    get_versioned_viewname method.
    """
    separator: str = '#'
